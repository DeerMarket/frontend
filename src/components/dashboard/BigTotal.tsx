...
export default function BigTotal({
  total,
  title,
  icon,
  currency,
  ...
  title?: string;
  icon?: any;
  currency?: "near" | string;
}) {
  let fontsize =
    total?.length > 12 ? 3 : total?.length > 8 ? 4 : total?.length > 5 ? 5 : 5;
  return (
    <Box
      sx={{
        flex: 1,
        ...
        ":hover": {
          variant: "box.dashboardCardHover",
        },
      }}
    >
      <Box>{icon}</Box>
      <Text
        sx={{
          fontSize: [fontsize - 1, fontsize, fontsize, fontsize + 1],
          fontWeight: 800,
          display: "flex",
          gap: 2,
          alignItems: "center",
